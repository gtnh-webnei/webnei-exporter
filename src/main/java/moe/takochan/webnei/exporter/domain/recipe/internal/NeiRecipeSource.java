 ...
 * {@link RecipeDomainStore#register}，由 domain 内部决定解析成什么；当前解析为 category，后续会展开 recipe。
 *
 * <p>
 ...
 * </ul>
 * 四个列表都会被扫描，确保普通与序列化注册的查合成、查用途 handler 都能进入 recipe store。
 */
public final class NeiRecipeSource {

    /** 扫描 NEI 四个 handler 列表，把每个 handler 交给 recipe store。 */
    public void collect(RecipeDomainStore store) {
