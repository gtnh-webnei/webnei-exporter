...
/** item 数据域中间模型，包含 item、variant、tooltip snapshot、tool class 与 list entry 行。 */
@Getter
public final class ItemExportModel implements IExportModel {
    ...
    /** 各受支持键态下的 tooltip 快照。 */
    private final List<ItemTooltipSnapshotRow> tooltipSnapshots;
    ...
    public ItemExportModel(List<ItemRow> items, List<ItemVariantRow> variants,
        List<ItemTooltipSnapshotRow> tooltipSnapshots, List<ItemToolClassRow> toolClasses,
        List<ItemListEntryRow> listEntries) {
        this.items = Collections.unmodifiableList(items);
        this.variants = Collections.unmodifiableList(variants);
        this.tooltipSnapshots = Collections.unmodifiableList(tooltipSnapshots);
