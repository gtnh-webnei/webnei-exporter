...
public final class NeiHandlerScanner {
    ...
    private List<NeiHandlerEntry> ensureUniqueCategoryIds(List<NeiHandlerEntry> entries) {
        ...
        for (NeiHandlerEntry entry : entries) {
            NeiHandlerDescriptor descriptor = entry.getDescriptor();
            if (counts.getOrDefault(descriptor.getResolvedCategoryId(), 0) <= 1) {
                out.add(entry);
                continue;
            }
            out.add(
                new NeiHandlerEntry(
                    copyWithCategoryId(
                        descriptor,
                        descriptor.getResolvedCategoryId() + "." + simpleClassName(descriptor.getHandlerClass())),
                    entry.getHandler()));
        }
        return out;
    }
    ...
    private static NeiHandlerDescriptor copyWithCategoryId(NeiHandlerDescriptor descriptor, String categoryId) {
        return new NeiHandlerDescriptor(
            descriptor.getRegistrationIndex(),
            ...
            descriptor.getIconStackId(),
            descriptor.getCatalystKey(),
            descriptor.getLoadedRecipeCount(),
            descriptor.getExtractionStatus(),
            descriptor.getReason());
    }
